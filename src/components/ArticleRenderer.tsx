...
export default function ArticleRenderer({
  ...
  const formatGluedText = (text: string, isArticleBody: boolean = false): React.ReactNode => {
    ...
    if (hasTable) {
        ...
        const isRef = /^\s*제\d+조(?:의\d+)?(?:(?:의(?!\d)|에|부터|까지|와|과|이나|나|를|을|은|는|이|가|,)|(?:\s|&nbsp;)*(?:제\d+[항호목]|등|관련|단서|본문|각\s*호))/.test(text.trim()) || 
                      /^\s*제\d+조(?:의\d+)?\s*[\[〔(（][^\]〕)）]+[\]〕)）](?:(?:의(?!\d)|에|부터|까지|와|과|이나|나|를|을|은|는|이|가|,)|(?:\s|&nbsp;)*(?:제\d+[항호목]|등|관련|단서|본문|각\s*호))/.test(text.trim());
        ...
        {lines.map((trimmedLine, idx) => {
          ...
          if (/^[①-⑳]/.test(trimmed)) {
             ...
             lineClass += " ml-4 block";
                     ...
                     !/^제\d+조(?:의\d+)?(?:(?:의(?!\d)|에|부터|까지|와|과|이나|나|를|을|은|는|이|가|,)|(?:\s|&nbsp;)*(?:제\d+[항호목]|등|관련|단서|본문|각\s*호))/.test(trimmed) && 
                     !/^제\d+조(?:의\d+)?\s*[\[〔(（][^\]〕)）]+[\]〕)）](?:(?:의(?!\d)|에|부터|까지|와|과|이나|나|를|을|은|는|이|가|,)|(?:\s|&nbsp;)*(?:제\d+[항호목]|등|관련|단서|본문|각\s*호))/.test(trimmed)) {
