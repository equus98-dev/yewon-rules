  ...
  return (
    <div className="w-screen h-screen flex flex-col bg-white select-none overflow-y-auto">
      ...
      <div className="flex-1 p-5 overflow-y-auto">
        <table className="w-full border-collapse text-[16px] shadow-sm rounded-lg overflow-hidden border border-slate-200">
          <colgroup>
            <col style={{ width: '28%' }} />
            <col style={{ width: '72%' }} />
          </colgroup>
          <tbody>
            ...
            <tr>
                ...
                {isAdmin && (
                  <div className="inline-flex items-center gap-1 ml-2">
                    <button
                      ...
                      className="text-[13px] bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded border border-emerald-200 hover:bg-emerald-100 transition-colors cursor-pointer font-bold shadow-sm inline-flex items-center gap-1"
                    ...
                    {revisions.length > 1 && selectedRev?.id === revisions[0]?.id && (
                      <button
                        ...
                        className="text-[13px] bg-red-50 text-red-600 px-2.5 py-1 rounded border border-red-200 hover:bg-red-100 transition-colors cursor-pointer font-bold shadow-sm inline-flex items-center gap-1"
              ...
              <td className="px-4 py-4 border-b border-slate-200 bg-white">
                <select 
                  ...
                  className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-[16px] bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-pointer font-medium shadow-sm"
                >
            ...
            <tr>
              ...
              <td className="px-4 py-4 border-b border-slate-200 font-bold text-slate-800 bg-white text-[16px]">
            ...
            {selectedRev && (
              <tr>
                ...
                <td className="px-4 py-4 border-b border-slate-200 bg-white">
                  <span className={`inline-flex items-center px-3.5 py-1 rounded-full text-[15px] font-bold shadow-sm ${
            ...
            {selectedRev && (
              <tr>
                ...
                <td className="px-4 py-4 border-b border-slate-200 text-slate-800 font-medium bg-white text-[16px]">
                  {isEditingDesc ? (
                    <input 
                      type="date" 
                      value={editEnactDate} 
                      onChange={(e) => setEditEnactDate(e.target.value)}
                      className="px-3 py-2 border border-slate-300 rounded-lg text-[16px] bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm font-medium"
                    />
                  ) : (
            ...
            <tr>
                ...
                {isAdmin && (
                  <>
                    ...
                    <button 
                      ...
                      className="ml-2 text-[13px] bg-amber-50 text-amber-700 px-2.5 py-1 rounded border border-amber-200 hover:bg-amber-100 transition-colors cursor-pointer font-bold shadow-sm inline-flex items-center gap-1 disabled:opacity-50"
                      title="원문파일 업로드"
                    >
              ...
              <td className="px-4 py-4 border-b border-slate-200 bg-white">
                {mainAttachments.length > 0 ? (
                        ...
                        <div key={idx} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-slate-200 bg-white hover:bg-blue-50 hover:border-blue-300 transition-all shadow-sm">
                          ...
                          {isAdmin && (
                            <button
                              ...
                              className="ml-1 text-slate-400 hover:text-red-600 font-bold text-[14px] px-1 cursor-pointer transition-colors"
                              title="파일 삭제"
                            >
                              ✕
                            </button>
                  ...
                  <div className="text-slate-400 text-[16px] italic font-medium">
                    등록된 전문 파일이 없습니다.
                ...
                <th className="bg-slate-50 text-left px-4 py-4 border-b border-slate-200 font-bold text-slate-700 align-top">
                  ...
                  {isAdmin && !isEditingDesc && (
                    <button 
                      ...
                      className="ml-2 text-[13px] bg-blue-50 text-blue-600 px-2.5 py-1 rounded border border-blue-200 hover:bg-blue-100 transition-colors cursor-pointer font-bold shadow-sm inline-flex items-center gap-1"
                      title="개정내용 및 날짜 편집"
                    >
                      ✏️ 편집
                    </button>
                  ...
                  {isEditingDesc ? (
                    <div className="flex flex-col gap-2.5">
                      <textarea
                        ...
                        className="w-full px-3.5 py-2.5 border border-slate-300 rounded-lg text-[16px] min-h-[160px] resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm leading-relaxed"
                        autoFocus
                      />
                      <div className="flex justify-end gap-2">
                        <button 
                          onClick={() => setIsEditingDesc(false)}
                          className="px-4 py-2 text-[15px] text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50 cursor-pointer font-bold shadow-sm"
                        >
                          취소
                        </button>
                        <button 
                          ...
                          className="px-4 py-2 text-[15px] text-white bg-blue-600 rounded-lg hover:bg-blue-700 cursor-pointer font-bold shadow-sm disabled:opacity-50 flex items-center gap-1"
                        >
                    ...
                    selectedRev.description ? (
                      <div className="whitespace-pre-wrap leading-relaxed text-[16px] font-medium">{selectedRev.description}</div>
                    ) : (
                      <div className="text-slate-400 text-[16px] italic font-medium">
                        등록된 개정내용이 없습니다.
