  ...
  if (loading) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center gap-4 bg-slate-50">
        ...
        <span className="text-slate-500 text-xs font-semibold">대시보드 통계 분석 로드 중...</span>
      </div>
    );
  }
  ...
  return (
    <div className="h-full overflow-y-auto p-8 bg-slate-50 scrollbar text-slate-800">
      <div className="max-w-6xl mx-auto space-y-8 pb-10">
        ...
        {/* 데이터베이스 오류 배너 */}
        {isDbError && (
          <div className="bg-red-50 border border-red-200 p-5 rounded-2xl text-xs text-red-700 font-bold select-none leading-relaxed shadow-sm">
          ...
          {statCards.map((card, idx) => (
            ...
            >
              <div className="space-y-1">
                <span className="text-[11px] text-slate-550 font-black tracking-wide">{card.label}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-between shadow-sm">
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-black bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded border border-blue-100 shrink-0 select-none">
                            {rev.departmentName}
                          </span>
                          <span className="text-[9px] font-black bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded border border-amber-100 shrink-0 select-none">
                            {rev.versionName}
                          </span>
                        </div>
                        <h4 className="text-[11px] font-black text-slate-800 mt-1.5 truncate">
                          {rev.title} <span className="text-slate-550 font-bold">({rev.ruleNumber})</span>
                        </h4>
                      </div>
                      <div className="text-right shrink-0 select-none">
                        <div className="text-[9px] text-slate-500 font-bold">{rev.enactmentDate}</div>
                        <div className="text-[8px] text-slate-400 font-bold mt-1">{rev.announcementNumber}</div>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 select-none">
              <Link
                href="/admin/rules"
                className="text-[11px] text-[#0c3161] font-black hover:text-blue-900 flex items-center justify-center gap-1 cursor-pointer active:scale-95 transition-all"
              >
                규정 리스트에서 조문 개정하기
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-sm">
          ...
          <Link
            href="/admin/editor"
            className="bg-[#0c3161] hover:bg-[#092244] text-white text-xs font-black px-5 py-2.5 rounded-xl shadow-lg shadow-[#0c3161]/20 hover:shadow-[#0c3161]/30 active:scale-95 transition-all select-none cursor-pointer text-center whitespace-nowrap"
          >
            입안편집기 런처 기동
          </Link>
        </div>
