async function run() {
  const res = await fetch("https://yewon-rules.pages.dev/api/rules/search?query=기자재선정위원회규정");
  const rules = await res.json();
  const rule = rules.bodyMatches?.[0] || rules.titleMatches?.[0] || rules.find(r => r.title.includes("기자재선정위원회규정"));
  if (!rule) {
     const allRes = await fetch("https://yewon-rules.pages.dev/api/rules/search?query=");
     const allRules = await allRes.json();
     const found = allRules.find(r => r.title.includes("기자재선정위원회규정"));
     if (found) {
