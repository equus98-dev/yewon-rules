...
const htmlPath = `E:\\예원예술대학교_규정관리시스템\\scratch\\hwp_html\\${filename}.htm`;

scriptLines.push(`if (Test-Path '${hwpPath}') {`);
scriptLines.push(`  $hwp.Open('${hwpPath}', "HWP", "forceopen:true")`);
scriptLines.push(`  $hwp.SaveAs('${htmlPath}', "HTM", "")`);
scriptLines.push(`  $hwp.Clear(1)`);
scriptLines.push(`}`);

scriptLines.push('$hwp.Quit()');

fs.writeFileSync('test_html.ps1', '\ufeff' + scriptLines.join('\n'), 'utf8');
console.log('test_html.ps1 created.');
