async function run() {
  ...
  for (const match of searchData.titleMatches) {
    ...
    for (const a of addendums) {
      ...
      // I will make sure the array format is correct for contentJson.
      // And I'll strip the leading "부칙 " if it got prepended previously.
      let newContent = origText.replace(/^부칙\s+/, "");
      ...
      await fetch(patchUrl, {
        ...
        body: JSON.stringify({
          ...
          contentJson: [{ type: "article", num: "", text: newContent }]
        })
      });
      console.log("Patched.");
    }
