...
const blockEndIdx = code.indexOf(endStr);
if (blockStartIdx !== -1 && blockEndIdx !== -1) {
    const block = code.substring(blockStartIdx, blockEndIdx + 11); // up to the end of '  }\n'
    code = code.substring(0, blockStartIdx) + code.substring(blockEndIdx + 11);
...
code = code.replace(
    /curHo = ""; curMok = "";\s*currentPath = `\$\{baseArticlePath\} \$\{curHang\}`\.trim\(\);/,
    'curHo = ""; curMok = ""; currentIndent = isArticleBody ? "40px" : "72px";\n               currentPath = `${baseArticlePath} ${curHang}`.trim();'
);

code = code.replace(
    /curMok = "";\s*currentPath = `\$\{baseArticlePath\} \$\{curHang\} \$\{curHo\}`\.replace\(\/\\s\+\/g, ' '\)\.trim\(\);/,
    'curMok = ""; currentIndent = isArticleBody ? "56px" : "88px";\n               currentPath = `${baseArticlePath} ${curHang} ${curHo}`.replace(/\\s+/g, \' \').trim();'
);

code = code.replace(
    /curMok = `\$\{numMatch\[1\]\.replace\('\.', ''\)\}목`;\s*currentPath = `\$\{baseArticlePath\} \$\{curHang\} \$\{curHo\} \$\{curMok\}`\.replace\(\/\\s\+\/g, ' '\)\.trim\(\);/,
